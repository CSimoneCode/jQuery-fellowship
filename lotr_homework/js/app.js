...
const makeMiddleEarth = () => {
  ...
  const $landsSection = $('<section />');
  $($landsSection).attr('id', 'middle-earth');
...
$($landArticle).wrapInner(`<h1>${lands[i]}</h1>`)
  //   3d. appends each land to the middle-earth section
  $($landsSection).append($landArticle);
}
};
...
const $theHobbits = $('#The-Shire').find('ul')
console.log($theHobbits)
$('#Rivendell').append($theHobbits);
$('#The-Shire').remove($theHobbits);
};
...
const $strider = $('aside').find('li').get(3);
console.log($strider)
...
const $gandalf = $('#the-fellowship').find('ul li:first');
const $resurrected = $($gandalf).text('Gandalf the White');
};
...
const thereAndBackAgain = () => {
  ...
  $('#gollum').remove()
  $('.baddy').remove()
  const $hobbits = $('body').find('.hobbit')
